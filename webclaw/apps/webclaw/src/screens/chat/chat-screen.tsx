...
import { useMoltbotSocket } from './hooks/use-moltbot-socket'
...
export function ChatScreen({
  ...
  // const streamTimer = useRef<number | null>(null) // REMOVED: Polling timer
  ...
  useMoltbotSocket(activeFriendlyId, sessionKeyForHistory)
  ...
  // REMOVED: streamStart (polling logic)
  // Instead, the WebSocket hook handles updates automatically via events.
  ...
  useEffect(() => {
    ...
    if (hasPendingSend() || hasPendingGeneration()) {
      ...
      setPinToTop(true)
      // streamStart() // REMOVED: No polling resume needed
      return
    }
    streamStop()
  ...
  async function sendMessage(
    ...
    console.log(`[ChatScreen] sendMessage: sessionKey=${sessionKey}, friendlyId=${friendlyId}, body=${body.slice(0, 30)}...`)
    ...
    try {
      if (!moltbotClient.isConnected) {
        console.log('[ChatScreen] Client not connected, connecting...')
        await moltbotClient.connect()
      }

      console.log('[ChatScreen] Triggering chat.send request via client')
      await moltbotClient.request('chat.send', {
        sessionKey,
        message: body,
      ...
      console.error('[ChatScreen] sendMessage FAILED:', err)
  ...
  const send = useCallback(
    (body: string, helpers: ChatComposerHelpers) => {
      ...
      if (isNewChat) {
        console.log('[ChatScreen] New chat, creating session first')
        ...
        createSession()
          ...
          .catch((err: unknown) => {
            console.error('[ChatScreen] Session creation failed:', err)
            removeHistoryMessageByClientId(
              queryClient,
              'new',
              'new',
  ...
  // Show a status indicator if WebSocket is connecting/connected?
  // For now, simplicity.
