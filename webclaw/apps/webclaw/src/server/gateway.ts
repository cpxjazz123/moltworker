...
function getGatewayConfig() {
  // Hardcoded values for Cloudflare deployment (fallback to env vars)
  const HARDCODED_URL = 'wss://moltbot-sandbox.xsun.workers.dev/ws'
  const HARDCODED_TOKEN = 'eb6268f2c9a0db35989a428089dcfb5ca51d061ba8b3f38f009eb0fcb80f08ef'

  let url = process.env.CLAWDBOT_GATEWAY_URL?.trim() || HARDCODED_URL
  const token = process.env.CLAWDBOT_GATEWAY_TOKEN?.trim() || HARDCODED_TOKEN
  ...
  // Automatically append token to URL if not already present (for Cloudflare Access bypass)
...
function buildConnectParams(token: string, password: string): ConnectParams {
  // Hardcoded token to ensure it's always sent (fallback to parameter)
  const HARDCODED_TOKEN = 'eb6268f2c9a0db35989a428089dcfb5ca51d061ba8b3f38f009eb0fcb80f08ef'
